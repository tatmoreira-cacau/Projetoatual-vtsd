...
**Visual aprovado (25/09): tema escuro com as cores do ebook, com cara de aplicativo.** Substitui o tema claro de 24/09. A referência de formato é o app da Google Play "Chocolate Cake Recipes: Baking" (Beast code Studios), mas **as cores e fontes são as do ebook**: fundo azul-marinho escuro `#0E1626` com cartões `#16243E`/`#1D2E4D`, texto creme `#F3ECE2`, títulos `#F3E3C9`, vinho `#8C3A3B` (botão principal, opção escolhida, círculos numerados do preparo), ouro `#B0863A` e `#D4A24E` (sobretítulos, aba ativa, detalhes), rosa `#D98B95` (líquido na pizza, ícones). Fontes: **Playfair Display** (títulos) e **Montserrat** (texto). Estrutura: barra no topo; **abas fixas embaixo** (Início · Monte · Receitas · Guia); passos numerados com botões em grade (textura em 3 cards com foto ou ilustração); receita com foto grande e título por cima, linha de informações e ingredientes com bolinha de marcar; aba **Guia** com a tabela de proporções (creme 35%, nacional/importado, mesmos números da Ganache Perfeita) e "Deu errado? Veja como consertar" (6 casos, base na Fase 2). Na impressão, volta para fundo branco. Largura máxima de 520 px. Conferido em 360 e 400 px em 26/09 (sem rolagem lateral; a barra de abas fica no rodapé. Em captura de "página inteira" ela aparece no meio, mas isso é só efeito da captura). Não trocar esse visual sem a Tatiana pedir.
