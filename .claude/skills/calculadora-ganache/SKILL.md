...
**MOTOR DE BALANCEAMENTO v2 (26/09, pedido da Tatiana: "refaça o motor baseado na literatura")** 
- O chocolate escuro **não se escolhe pelo nome** (amargo/meio amargo): a pessoa informa o **% de cacau do rótulo** (e, se quiser, a gordura em 100 g da tabela nutricional). Tipos: Amargo/meio amargo (escuro), Ao leite, Branco, Blend, Caramelo, Ruby.
- Base na literatura: Fase 1 (1.3 composição, 1.4 "olhar a composição, não o nome"), Fase 2 (2.2: manteiga de cacau dá firmeza, sólidos absorvem água), Contexto Mestre item 5 (percentuais funcionais, cada gordura separada).
- **Conta:** g de chocolate por g de água = PROPORÇÃO do chocolate de referência × MC da referência ÷ MC do chocolate da pessoa ÷ 0,58. MC = gordura que firma (manteiga de cacau + gordura vegetal). Referências importadas com ficha: 811 (54,5%, MC 36,6), Guanaja (70%, MC 42), 823 (33,6%, MC ~30), W2 (28%, MC 29,5); no escuro, a referência é interpolada entre 811 e Guanaja pelo % de cacau (abaixo de 54,5% é o 811). MC pelo % de cacau: `mcImp`/`mcNac` no fonte (nacional: Sicao 40% ~26, Harald Unique 70% ~38). Com gordura do rótulo: MC = gordura − gordura do leite (6 no ao leite e no branco).
- **Com o % de cacau padrão, o motor reproduz a tabela Ganache Perfeita** (o `testar.py` confere, com 2% de tolerância) e, no escuro, mais cacau sempre dá menos chocolate por creme (mais perto de 1 : 1).
- O aviso "+10% de chocolate branco de supermercado" saiu: o branco nacional já entra na conta.
- A tabela do capítulo (`ganache-perfeita-src.html`) ainda mostra "Amargo" e "Meio amargo"; mudar só se a Tatiana pedir.
